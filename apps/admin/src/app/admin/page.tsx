...
export default async function AdminDashboard(
    ...
    return (
        <div className="min-h-screen bg-[#F9FAFB] px-4 py-6 sm:px-6 sm:py-8">
            <div className="mx-auto max-w-7xl space-y-6">

                {/* PAGE TITLE */}
                <div>
                    <h1 className="text-xl font-extrabold text-gray-900">Transactions</h1>
                ...
                <form
                    ...
                    action="/admin"
                    method="GET"
                    className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-200 flex flex-col lg:flex-row items-end gap-3"
                >
                    <div className="w-full lg:flex-[2]">
                        ...
                        <input
                            ...
                            className="block h-11 w-full rounded-xl border-0 px-4 text-sm text-gray-900 ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-[#1A3C9F] outline-none placeholder:text-gray-400"
                        />
                    </div>
                    ...
                    <div className="w-full lg:flex-1">
                        <label className="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-gray-500">Zone</label>
                        <CustomListbox name="villageId" options={VILLAGE_OPTIONS} defaultValue={params.villageId} placeholder="Toutes les zones" />
                    </div>
                    <div className="flex w-full lg:w-auto gap-2">
                        <Link
                            href="/admin"
                            className="flex h-11 items-center justify-center rounded-xl border border-gray-200 px-4 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors w-full lg:w-auto"
                        >
                            Effacer
                        </Link>
                        <button
                            type="submit"
                            className="flex h-11 items-center justify-center rounded-xl bg-[#1A3C9F] px-6 text-sm font-bold text-white hover:bg-[#142E7B] transition-colors shadow-sm w-full lg:w-auto"
                        >
                            Filtrer
                        </button>
                    </div>
                ...
                <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200">
                        ...
                        <table className="min-w-full divide-y divide-gray-100 text-left text-sm">
                            <thead>
                                <tr className="bg-gray-50/60">
                                    <th className="px-5 py-4 font-bold text-gray-700 whitespace-nowrap">Client</th>
                                    <th className="px-5 py-4 font-bold text-gray-700 whitespace-nowrap">Zone</th>
                                ...
                                {payments.map(payment => (
                                    <tr key={payment.id} className="hover:bg-blue-50/40 transition-colors">
                                        <td className="whitespace-nowrap px-5 py-4">
                                            ...
                                            <p className="text-xs text-gray-400 mt-0.5">{payment.client.phone ?? ''}</p>
                                        </td>
                                        <td className="px-5 py-4">
                                            <span className="font-semibold text-gray-700">{payment.client.village.titre ?? '—'}</span>
                                        </td>
                                        <td className="px-5 py-4">
                                            <span className="font-medium text-gray-600">{PROVIDER_LABELS[payment.provider]}</span>
                                        </td>
                                        <td className="px-5 py-4 font-extrabold text-gray-900 whitespace-nowrap">
                                            {payment.amount.toLocaleString('fr-FR')} <span className="text-xs font-bold text-gray-500">FCFA</span>
                                        </td>
                                        <td className="px-5 py-4">
                                    ...
                                    <tr>
                                        <td colSpan={6} className="px-6 py-16">
                                            <div className="flex flex-col items-center justify-center text-center">
                                                ...
                                                <p className="font-bold text-gray-900">Aucune transaction trouvée</p>
                                                <p className="mt-1 text-sm text-gray-500">Modifiez vos critères de filtre.</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                    ...
                    {/* PAGINATION */}
                    {totalPages > 1 && (
                        <div className="border-t border-gray-100 px-5 py-3 flex items-center justify-between gap-4">
                            <p className="text-xs text-gray-500">
                                Page <span className="font-bold text-gray-700">{page}</span> sur <span className="font-bold text-gray-700">{totalPages}</span>
                                {' '}·{' '}
                                <span className="font-bold text-gray-700">{totalCount}</span> résultat{totalCount !== 1 ? 's' : ''}
                            </p>
                            <div className="flex items-center gap-2">
                                {page > 1 ? (
                                    ...
                                    <span className="flex h-8 w-8 items-center justify-center rounded-lg border border-gray-100 text-gray-300">
                                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                                        </svg>
                                    </span>
...
function SortHeader({
    ...
    return (
        <th className="px-5 py-4 font-bold text-gray-700 group select-none whitespace-nowrap">
            <Link href={href} className="flex items-center gap-1.5 hover:text-[#1A3C9F] transition-colors">
                {label}
...
function StatCard({ label, value, accent }: { label: string; value: string; accent: boolean }) {
    return (
        <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
            ...
            <p className={`mt-2 text-2xl font-extrabold ${accent ? 'text-[#1A3C9F]' : 'text-gray-900'}`}>
                {value}
            </p>
        </div>
    );
